...
from viz.operations import flattened_pixel_locations_to_u_v


def plot_heatmaps(img_a_rgb, img_b_rgb, embed_a, embed_b,
                  uv_a=None, uv_b=None, circ_color='g', show=True,
                  title=None):
    ...
    embed_a = np.repeat(np.repeat(embed_a, scale_factor, axis=1),
                        scale_factor, axis=0)
    embed_b = np.repeat(np.repeat(embed_b, scale_factor, axis=1),
                        scale_factor, axis=0)
    ...
    for ax, img, pixel_loc in zip(axes[0:], images, pixel_locs):
        ...
        if uv_a is not None and uv_b is not None:  # TODO: flatten nested if
            if isinstance(pixel_loc[0], (int, float)):
                circ = Circle(pixel_loc, radius=3, facecolor=circ_color,
                              edgecolor='white', fill=True, linewidth=2.0,
                              linestyle='solid')
                ax.add_patch(circ)
            else:
                for x, y in zip(pixel_loc[0], pixel_loc[1]):
                    circ = Circle((x, y), radius=3, facecolor=circ_color,
                                  edgecolor='white', fill=True, linewidth=2.0,
                                  linestyle='solid')
                    ax.add_patch(circ)
...
def cross_debug_plot(image_a_rgb, image_b_rgb,
    ...
    plot_correspondences_direct(
        image_a_rgb, image_a_depth_numpy,
        image_b_rgb, image_b_depth_numpy,
        plot_blind_uv_a, plot_blind_uv_b,
        circ_color='k', show=True, title="Blind non-matches")


def debug_plots(image_a_rgb, image_b_rgb, image_height,
                image_a_depth_numpy, image_b_depth_numpy,
    ...
    blind_uv_a = flattened_pixel_locations_to_u_v(
...
def plot_correspondences(images, uv_a, uv_b, use_previous_plot=None,
                         circ_color='g', show=True, title=None, masks=None):
    ...
    for ax, img, pixel_loc, mask in zip(axes[0:], images, pixel_locs, masks):
        ...
        if isinstance(pixel_loc[0], int) or isinstance(pixel_loc[0], float):
            circ = Circle(pixel_loc, radius=3, facecolor=circ_color,
                          edgecolor='white', fill=True, linewidth=1.0,
                          linestyle='solid')
            ax.add_patch(circ)
        else:
...
def plot_correspondences_direct(img_a_rgb, img_a_depth, img_b_rgb, img_b_depth,
                                uv_a, uv_b, use_previous_plot=None,
                                circ_color='g', show=True, title=None,
                                masks=None):
    """
    ...
    return plot_correspondences(images, uv_a, uv_b,
                                use_previous_plot=use_previous_plot,
                                circ_color=circ_color, show=show, title=title,
                                masks=masks)
...
def subsample_tuple(uv, num_samples):
    ...
    return (torch.index_select(uv[0], 0, indexes_to_keep),
