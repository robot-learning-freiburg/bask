...
def activation_map(softmax_activations, keypoints=None):
    if keypoints is not None:
        keypoints_x, keypoints_y = \
            np.array_split(keypoints.squeeze(0).numpy(), 2)
    for i, density in enumerate(softmax_activations.squeeze(0).numpy()):
        ...
        plt.scatter([(kp[0] + 1) * (W-1)/2 + .5],
                    [(kp[1] + 1) * (H-1)/2 + .5], c='r', s=50)

        g.ax_marg_y.barh(np.arange(0.5, H),
                         density.sum(axis=1), color='navy')
        g.ax_marg_x.bar(np.arange(0.5, W),
                        density.sum(axis=0), color='navy')
