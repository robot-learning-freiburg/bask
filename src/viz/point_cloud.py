...
def point_cloud_pair_with_mutuals(points_1, points_2, points_m):
    ...
    ax.scatter([p[0] for p in points_1], [p[1] for p in points_1],
               [p[2] for p in points_1], marker='o', alpha=.2, s=.25,
               color='blue')
    ax.scatter([p[0] for p in points_2], [p[1] for p in points_2],
               [p[2] for p in points_2], marker='o', alpha=.2, s=.25,
               color='green')
    ax.scatter([p[0] for p in points_m], [p[1] for p in points_m],
               [p[2] for p in points_m], marker='o', color='red', alpha=1,
               s=100)
    plt.show()
...
def create_series(array_list, name=None):
    ...
    for i in range(num_images):
        ...
        points = array_list[i]
        alpha = min(1, 1/int(len(points)**(1./3.)))
        ax.scatter([p[0] for p in points], [p[1] for p in points],
                   [p[2] for p in points], marker='o', alpha=alpha, s=.25,
                   color='green')
        ax.axis('off')
