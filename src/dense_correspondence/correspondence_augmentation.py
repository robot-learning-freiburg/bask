...
def domain_randomize_background(image_rgb, image_mask):
    ...
    three_channel_mask_complement = torch.ones_like(
        three_channel_mask) - three_channel_mask
    random_rgb_image = get_random_image(
        image_rgb.shape, device=image_rgb.device)
...
def get_random_image(shape, device='cpu'):
    ...
    return torch.clamp(rand_image/255, min=0., max=1.)
...
def add_noise(rgb_image):
    ...
    pos = get_random_entire_image(
        rgb_image.shape, max_noise_to_add_or_subtract)
    neg = get_random_entire_image(
        rgb_image.shape, max_noise_to_add_or_subtract)
    return rgb_image + pos - neg


def merge_images_with_occlusions(image_a, image_b, mask_a, mask_b,
                                 depth_a, depth_b,
    ...
    if True:
        ...
        background_image, background_mask, background_matches_pair = \
            image_a, mask_a, matches_pair_a
        foreground_image, foreground_mask, foreground_matches_pair = \
            image_b, mask_b, matches_pair_b
    else:
        foreground = "A"
        background_image, background_mask, background_matches_pair = \
            image_b, mask_b, matches_pair_b
    ...
    return merged_image, merged_masked, merged_depth, matches_a, \
        associated_matches_a, matches_b, associated_matches_b
...
def random_apply_random_crop(images_a, images_b, uv_a, uv_b,
...
def random_crop(images_a, uv_a, uv_b, crop_size=(215, 215),
                sample_crop_size=False):
    h, w = images_a[0].shape[-2:]

    if sample_crop_size:
        crop_size = tuple((random.randint(crop_size[0], h),
                           random.randint(crop_size[1], w)))
    ...
    cropped_images = [i[..., hc:hc + crop_size[0], wc:wc + crop_size[1]]
                      for i in images_a]
    ...
    if uv_a is not None:
        ...
        mask = mask_u & mask_v
        mask_cropped_pixel_pos = tuple((uv_a[0][mask], uv_a[1][mask]))

        translated_pixel_pos = tuple((mask_cropped_pixel_pos[0] - wc + wt,
                                      mask_cropped_pixel_pos[1] - hc + ht))

        filtere_uv_b = tuple((uv_b[0][mask], uv_b[1][mask]))
