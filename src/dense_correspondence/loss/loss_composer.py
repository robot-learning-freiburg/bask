...
from dataset.dc import DcDatasetDataType, DenseCorrespondenceDataset
...
def get_loss(pixelwise_contrastive_loss, match_type,
    ...
    if match_type == DcDatasetDataType.SINGLE_OBJECT_WITHIN_SCENE:
        ...
        return get_within_scene_loss(
            pixelwise_contrastive_loss, image_a_pred, image_b_pred,
            matches_a, matches_b, masked_non_matches_a, masked_non_matches_b,
            background_non_matches_a, background_non_matches_b,
            blind_non_matches_a, blind_non_matches_b)
...
def get_within_scene_loss(
    ...
    match_loss, masked_non_match_loss, num_masked_hard_negatives, \
        match_descriptor_dist, masked_non_match_descriptor_dist = \
    ...
    if not DenseCorrespondenceDataset.is_empty(blind_non_matches_a):
        blind_non_match_loss, num_blind_hard_negatives, \
            ...
            pixelwise_contrastive_loss.non_match_loss_descriptor_only(
                ...
                blind_non_matches_b, M_descriptor=pcl._config["M_masked"])

        descriptor_distances['blind_nm_dist'] = blind_non_match_descriptor_dist

    total_num_hard_negatives = \
        num_masked_hard_negatives + num_background_hard_negatives
    ...
    if pcl._config["scale_by_hard_negatives"]:
        ...
        blind_non_match_loss_scaled = \
            blind_non_match_loss*1.0/num_blind_non_matches
...
def get_within_scene_loss_triplet(
    ...
    masked_triplet_loss =\
        pixelwise_contrastive_loss.get_triplet_loss(
            image_a_pred, image_b_pred, matches_a,
    ...
    background_triplet_loss =\
        pixelwise_contrastive_loss.get_triplet_loss(
            image_a_pred, image_b_pred, matches_a,
            matches_b, background_non_matches_a, background_non_matches_b,
            pcl._config["alpha_triplet"])
...
def get_different_object_loss(
    ...
    scale_by_hard_negatives = \
        pixelwise_contrastive_loss.config[
            "scale_by_hard_negatives_DIFFERENT_OBJECT"]
    blind_non_match_loss = zero_loss()
    if not (DenseCorrespondenceDataset.is_empty(blind_non_matches_a)):
        M_descriptor = pixelwise_contrastive_loss.config["M_background"]

        blind_non_match_loss, num_hard_negatives, non_match_descriptor_dist = \
            pixelwise_contrastive_loss.non_match_loss_descriptor_only(
                image_a_pred, image_b_pred, blind_non_matches_a,
                blind_non_matches_b, M_descriptor=M_descriptor)
