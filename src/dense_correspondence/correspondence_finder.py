...
from utils.torch import (conjugate_quat, invert_homogenous_transform,
                         invert_intrinsics, quaternion_multiply,
...
def find_best_match(pixel_a, res_a, res_b):
    ...
    norm_diffs = cdist(np.expand_dims(descriptor_at_pixel, axis=0),
                       np.reshape(res_b, (-1, D)),
                       'euclidean')[0]
    ...
    best_match_xy = np.unravel_index(
        best_match_flattened_idx, (height, width))
...
def get_norm_diffs(pixel, descriptor):
...
def get_norm_diffs_to_ref(descriptor_at_pixel, descriptor):
    height, width, D = descriptor.shape
...
def random_sample_from_masked_image_torch(img_mask, num_samples):
    ...
    uv_vec = flattened_pixel_locations_to_u_v(
...
def batch_find_pixel_correspondences(img_a_depth, img_a_pose, img_b_depth,
                                     img_b_pose, uv_a=None, num_attempts=20,
                                     device='cpu', img_a_mask=None,
                                     K_a=None, K_b=None,
                                     obj_pose_a=None, obj_pose_b=None,
                                     object_no=None):
    """
    ...
    depth_vec = torch.index_select(
        img_a_depth, 0, uv_a_vec_flattened)*1.0
    ...
    if obj_pose_a is not None and obj_pose_b is not None:
        ...
        quat_diff = quaternion_multiply(conjugate_quat(ref_rot), cur_rot)
        rel_rot_matrix = quaternion_to_matrix(quat_diff)

        move_back = torch.eye(4, device=obj_pose_a.device)
        move_back[:3, 3] = - ref_shift
        rel_rot = torch.eye(4, device=obj_pose_a.device)
        rel_rot[:3, :3] = rel_rot_matrix
        move_forth = torch.eye(4, device=obj_pose_a.device)
    ...
    depth2_vec = torch.where(depth2_vec < z2_vec, zeros_vec,
