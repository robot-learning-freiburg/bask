...
from env.observation import MaskObservation
from env.trajectory import BaseEnvironment, CustomEnv


class MaskEnv(CustomEnv):
    def __init__(self, config):
        ...
        obs_config = ObservationConfig(
            left_shoulder_camera=CameraConfig(
                ...
                mask=True, render_mode=RenderMode.OPENGL,
    ...
    def obs_split(self, obs, live=True):
        ...
        # if not interacting live with the env (eg. when using demos), the
        # camera pose cannot be extracted. In that case we do not downsample,
        # so it should be fine to just set it to zero.
        ...
        return MaskObservation(gripper_pose, proprio_obs, wrist_pose,
