...
from models.monet import networks
...
class MONetModel(nn.Module):
    def __init__(self, config):
        ...
        self.attention_net = networks.AttentionNet(config["image_channels"], 1)

        for i, d in enumerate(config["latent_dims"]):
            setattr(self, 'bvae' + str(i),
                    networks.VAE(input_channels=config["image_channels"],
                                 latent_dim=d, sigma_bg=config["sigma_bg"],
    ...
    def encode(self, batch):
        ...
        for k in range(self.no_slots):
            if k != self.no_slots - 1:
                log_alpha_k, alpha_logits_k = self.attention_net(
                    batch, log_s_k)
    ...
    def forward(self, batch):
        ...
        for k in range(self.no_slots):
            ...
            # parameters
            m_tilde_k_logits, x_mu_k, x_logvar_k, z_mu_k, z_logvar_k = \
                bvae(batch, log_m_k, k == 0)
            ...
            loss_E += -0.5 * \
                (1 + z_logvar_k - z_mu_k.pow(2) - z_logvar_k.exp()).sum()
