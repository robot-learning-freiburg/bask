...
from models.bvae.base import BaseVAE
...
class BetaVAE(BaseVAE):
    def __init__(self, config, **kwargs) -> None:
        ...
        for i, o, k, s, p in zip(
                ...
                config["hidden_dims"], config["filter_size"],
                config["stride"], config["padding"]):
            modules.append(
                nn.Sequential(
                    nn.Conv2d(in_channels=i, out_channels=o,
        ...
        self.fc_encoder = nn.Sequential(  # 8*8 for image resolution 128
            nn.Linear(config["hidden_dims"][-2]*8*8,
                      config["hidden_dims"][-1]),
            nn.ReLU())
        self.fc_mu = nn.Linear(
            config["hidden_dims"][-1], config["latent_dim"])
        ...
        for i, o, k, s, p in reversed(list(
            zip(config["hidden_dims"][1:], config["hidden_dims"][:-1],
                config["filter_size"][1:], config["stride"][1:],
                config["padding"][1:]))):
            modules.append(
                nn.Sequential(
        ...
        self.final_layer = nn.Sequential(
            nn.ConvTranspose2d(config["hidden_dims"][0],
            ...
            nn.ReLU(),
            nn.Conv2d(config["hidden_dims"][0],
