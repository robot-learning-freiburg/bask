...
def hard_pixels_to_3D_world(
    ...
    for i in range(N):
        for j in range(k):
            ...
            u_pixel_coordinate = ((u_normalized_coordinate/2.0 + 0.5)
                                  * img_width).long().detach().cpu().numpy()
            v_pixel_coordinate = ((v_normalized_coordinate/2.0 + 0.5)
                                  * img_height).long().detach().cpu().numpy()
...
def raw_pixels_to_camera_frame(
    ...
    for i in range(N):
        for j in range(k):
            ...
            # print(uv, z, K_one, camera_to_world_one)
            point_3d_cam = pinhole_projection_image_to_camera_coordinates(
                uv, z, K_one)
...
def compute_expected_z(softmax_activations, depth):
    ...
    for i in range(N):
        one_expected_z = torch.sum(
            (softmax_activations[i]*downsampled_depth[i, 0]).unsqueeze(0),
            dim=(2, 3))  # 1, nm
...
def soft_pixels_to_3D_world(
    ...
    for i in range(N):
        for j in range(k):
            ...
            this_depth = (depth[i]).unsqueeze(0).unsqueeze(
                1).to(device)  # convert to meters, from millimeters
            z = compute_expected_z(sm_activations[i, j].unsqueeze(
                0).unsqueeze(0), this_depth).cpu().detach().numpy()
...
def rigid_transform(xyz, transform):
    ...
    """
    xyz_h = np.concatenate([xyz, np.ones((*xyz.shape[:-1], 1),
...
def project_onto_cam(point_cloud, depth_im, cam_intr, cam_pose):
    ...
    for b in range(pcs[0]):
        cam_pts[b] = rigid_transform(point_cloud[b],
                                     np.linalg.inv(cam_pose[b]))
    pix_z = cam_pts[:, :, 2]
    ...
    valid_pix = np.logical_and(
        pix_x >= 0, np.logical_and(
            pix_x < im_w, np.logical_and(
                pix_y >= 0, np.logical_and(pix_y < im_h, pix_z > 0))))
...
def append_depth_to_uv(
    ...
    for i in range(N):
        for j in range(k):
            ...
            v_normalized_coordinate = y_vision[i, k+j]
            u_pixel_coordinate = ((u_normalized_coordinate/2.0 + 0.5)
                                  * img_width).long().detach()
            v_pixel_coordinate = ((v_normalized_coordinate/2.0 + 0.5)
                                  * img_height).long().detach()
