...
import models.keypoints.resnet as models
...
def get_fcn(config):
    ...
    if config['vision_net'].startswith("Resnet"):
        ...
        dino = config.get("dino_pretrained")
        supervised = config.get("imagenet_pretrained")
...
def adjust_input_image_size_for_proper_feature_alignment(input_img_batch, output_stride=8):
    ...
    # https://github.com/tensorflow/models/blob/master/research/slim/nets/resnet_v1.py#L159
    new_spatial_dims = np.ceil(
        input_spatial_dims / output_stride).astype(np.int) * output_stride + 1
    ...
    input_img_batch_new_size = nn.functional.interpolate(
        input=input_img_batch, size=new_spatial_dims,
