...
class DiscreteFilter:
    def __init__(self, config: dict) -> None:
        ...
        self.n_cams = config['obs_config']['n_cams']
        ...
        self.use_motion_model = encoder_config.get('use_motion_model')
    ...
    def get_prior(self, rgb: tuple, depth: tuple, extr: tuple, intr: tuple,
                  ) -> tuple:
        ...
        motion_model = tuple(
            self.get_motion_model(
                ...
                self.last_d[i], self.last_int[i], self.last_ext[i])
            for i in range(n_cams))

        prior_after_motion = tuple(
            self.apply_motion_model(prior[i], motion_model[i])
            for i in range(n_cams))

        return prior_after_motion

    def get_motion_model(self, depth_a, intr_a, extr_a, depth_b, intr_b,
                         extr_b) -> torch.Tensor | None:
        """
