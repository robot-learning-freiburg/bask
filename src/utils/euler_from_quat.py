...
def _compute_euler_from_quat(quat, seq, extrinsic=False):
    ...
    angles = torch.empty((num_rotations, 3))
    eps = 1e-7

    for ind in range(num_rotations):  # TODO: vectorize this
        _angles = angles[ind, :]
        ...
        _angles[1] = torch.arccos(2*(a**2 + b**2) / n2 - 1)

        # ... and check if equalt to is 0 or pi, causing a singularity
        safe1 = abs(_angles[1]) >= eps
        safe2 = abs(_angles[1] - torch.pi) >= eps
        safe = safe1 and safe2

        # Step 4
        # compute first and third angles, according to case
        if safe:
            half_sum = torch.arctan2(b, a)  # == (alpha+gamma)/2
            half_diff = torch.arctan2(-d, c)  # == (alpha-gamma)/2
        ...
        for i_ in range(3):
            if _angles[i_] < -torch.pi:
                _angles[i_] += 2 * torch.pi
            elif _angles[i_] > torch.pi:
                _angles[i_] -= 2 * torch.pi

        # for Tait-Bryan angles
        if not is_proper:
            _angles[2] *= sign
            _angles[1] -= torch.pi / 2
...
def euler_from_quat(quat, seq='xyz'):
    ...
    angles_flat = _compute_euler_from_quat(quat_flat, seq, extrinsic)

    angles = angles_flat.reshape(quat_shape[:-1] + (3,))
