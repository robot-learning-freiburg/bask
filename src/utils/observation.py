...
ALL_CAMERA_ATTRIBUTES = RAW_CAMERA_ATTRIBUTES | CAMERA_MASK_ATTRIBUTES | \
    CAMERA_EMBEDDING_ATTRIBUTES
...
CAMERA_OBS_ATTRIBUTES = list(RAW_CAMERA_ATTRIBUTES.keys()) + \
    list(CAMERA_EMBEDDING_ATTRIBUTES.keys()) + ['mask']
...
def collate(observations: tuple[TensorDictBase], pad: bool = False
            ) -> TensorDictBase:
    """
...
def get_cam_attributes(cam_name: str, mask_type: MaskTypes | None,
                       skip_rgb=False, shorten_cam_names=True
                       ) -> dict[str, str]:
    """
    ...
    cam_attributes = {
        attr: make_cam_attr_name(cam, RAW_CAMERA_ATTRIBUTES[attr])
        for attr in cam_attr
    }

    if mask_type not in [MaskTypes.NONE, None]:
        mask_attr_name = "mask_" + mask_type.value  # type: ignore
        cam_attributes.update(
            {'mask': make_cam_attr_name(cam, mask_attr_name)})
...
def get_idx_by_pose_difference_threshold(
        poses: Sequence[np.ndarray], dist_threshold: float,
        angle_threshold: float) -> list[int]:
    """
...
def downsample_to_target_freq(traj: torch.Tensor, target_freq: int,
                              source_freq=20) -> torch.Tensor:
    """
...
def downsample_tensordict_by_idx(traj: TensorDictBase, indeces: list[int]
                                 ) -> TensorDictBase:
    """
...
def make_tensorclass(name, tensor_fields=None, other_fields=None,
                     methods=None, properties=None):
    """
...
SceneObservation = make_tensorclass(
    "SceneObservation",
    tensor_fields=GENERIC_STATE_ATTRIBUTES + OPTIONAL_STATE_ATTRIBUTES,
    other_fields={
        'cameras': TensorDict
    },
    methods={
...
def dropout_single_camera(obs: SingleCamObservation,  # type: ignore
...
def random_obs_dropout(obs: SceneObservation,   # type: ignore
                       p: float | None = None,
                       drop_all: bool = False,
                       attributes: tuple[str] = tuple(("rgb", "depth"))
